...
Music is **not bundled**. The optional `Darkmoonshine_Theme/bgm.mp3` is
user-supplied on your own SD; the packager excludes it even if a personal
copy exists in a developer's checkout. This packaging correction does not
change the launcher or game payload, so it does not invalidate existing
V1.0.0 archives.
