...
custom theme files, settings and normal game saves. Make **fresh RC4 states**:
...
7. **Named SD export/import.** Name/export a fresh RC4 state A. Save a different
...
8. **Failed import preserves memory.** With a working RC4 state in memory,
...
9. **Reusable reboot state.** Export a fresh RC4 state marked REBOOT READY.
...
RC4 retains one RAM slot plus named SD archives. The second complete state
